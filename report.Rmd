...
p1 = 
  cv_res %>% 
    ...
    ggplot(aes(x=model, y=RMSE, fill = model)) + geom_boxplot() + ggtitle('Cross Validation RMSE')
...
y_compare_df %>% 
  ...
  theme(legend.position="right") + ggtitle(paste0('Testset RMSE of LASSO:', as.character(as.integer(rmse_lasso))))
...
y_compare_df %>% 
  ...
  ggplot(aes(x = num, y = price), color = type) + 
  geom_point(aes(colour = type)) + 
  theme(legend.position="right") + ggtitle(paste0('Testset RMSE of TREE:', as.character(as.integer(rmse_tree))))
...
y_compare_df %>% 
  ...
  ggplot(aes(x = num, y = price), color = type) + 
  geom_point(aes(colour = type)) + 
  theme(legend.position="right") + ggtitle(paste0('Testset RMSE of GBM:', as.character(as.integer(rmse_gbm))))
...
p2 =
  imp_tree_df %>% 
      ...
      geom_bar(stat="identity") + theme(legend.position="none",axis.text.x = element_text(angle = 21)) + labs(x="variable", y = 'importance')



imp_gbm = varImp(gbm.train, scale = TRUE)
imp_gbm_df = 
